	...
	protected override void Load()
	{
		...
		AddNpc(20060, L("[Plateau-Watch] Mindaugas"), "f_tableland_74", -700, -250, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you open the path for the caravans?"),
		...
		//-------------------------------------------------------------------------
		AddNpc(20114, L("[Alchemist] Tilda"), "f_tableland_74", 150, -1050, 0, async dialog =>
		{
			var character = dialog.Player;
			var questId = new QuestId("f_tableland_74", 1002);

			dialog.SetTitle(L("Tilda"));

			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("Purple Keparis nest the rock-folds. Kill twenty-five, bring seven flesh-cuts for reagent work."));

				var response = await dialog.Select(L("Will you bring me the cuts?"),
		...
		//-------------------------------------------------------------------------
		AddNpc(153142, L("[Aether-Scholar] Justina"), "f_tableland_74", 1250, 1250, 270, async dialog =>
		{
			var character = dialog.Player;
			var questId = new QuestId("f_tableland_74", 1003);

			dialog.SetTitle(L("Justina"));
		...
		AddNpc(20116, L("[Caravan-Master] Jurgita"), "f_tableland_74", -1650, -400, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A caravan-master inspects an iron wheel-rim by candle-light, frowning at a hairline crack*{/}"));
				await dialog.Msg(L("My salt-run goes from the salt-flats to the upper-terrace camp every fortnight. Three wagons, six drovers, twelve oxen. Same route my father drove, and his father before him."));
				await dialog.Msg(L("Tiny Brown Mages drift through the upper terrace and erase the wheel-rut chalk every time they pass. The drovers navigate by those ruts - they don't have time to re-survey the route every trip. No ruts, no salt-run. No salt-run, no bread on the upper terrace by week's end."));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*She hands over a stub of bright-yellow caravan-chalk*{/}"));
						...
						await dialog.Msg(L("Then four ruts. Wagon-tracks faded along the salt-route. Yellow chalk because the upper-terrace wind doesn't strip yellow as fast as white. Press hard - drovers need to read it from a wagon-bench at speed."));
						break;

					case "info":
						await dialog.Msg(L("Salt cures meat for winter, preserves cheese, keeps well-water sweet, ferments the cabbage. The upper-terrace camp goes through a barrel of salt every two days. Without the salt-run, they slaughter their preserved-stores in a week."));
						await dialog.Msg(L("Then they slaughter the herds. Then the breeding-stock. Then they go home. The camp dies from the salt-shortage, not the monsters."));
						break;

					case "leave":
						await dialog.Msg(L("Wagons stay parked, the camp burns through its preserved-stores, the upper terrace empties for winter. I'll hire a wheelwright to fix this rim and walk the salt-route with my drovers myself if I have to. I'd rather not."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*Her drovers are harnessing the lead ox-team, wheels squealing forward against the rut-chalk*{/}"));
					await dialog.Msg(L("Wheels rolling, ruts re-chalked yellow. Drovers can read the line from the wagon-bench again. Salt at the upper-terrace camp by tomorrow noon."));
					await dialog.Msg(L("Caravan-master's purse, plus a salt-brick from the first cart. Tradition for whoever clears the route. Boil it down and trade it - upper-terrace salt-brick fetches double in Klaipeda."));
					character.Quests.Complete(questId);
				}
				else if (!killObj.Done)
				{
					await dialog.Msg(L("Eighteen Mages first. Re-marking ruts while they're still drifting the terrace just gives them fresh chalk to mess with."));
				}
				else
				{
					await dialog.Msg(L("Mages cleared. Now the four ruts - yellow chalk, press hard, drovers read at speed. Salt-cart leaves at first light."));
				}
			}
			else if (character.Quests.HasCompleted(questId))
			{
				await dialog.Msg(L("Salt at the camp on time. Cooks salted the winter-meat the same evening. Cheese rounds went into brine the next morning. The upper-terrace winter is provisioned because of you. The cooks don't know your name. The camp will eat well anyway."));
			}
		});
		...
		//-------------------------------------------------------------------------
		AddNpc(20117, L("[Bounty-Sergeant] Tadas"), "f_tableland_74", 1000, -130, 0, async dialog =>
		{
			var character = dialog.Player;
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A bounty-sergeant has a faded warrior-codex unrolled and held open with two stones*{/}"));
				await dialog.Msg(L("The Blue Harugal Elite leads the pack on the steel heights. Old, scarred, raised on the Harugal warrior-code - the one in this codex. He'll answer a formal challenge, but he won't fight a hunter who hasn't issued one."));
				await dialog.Msg(L("Three iron-shod challenge-stakes planted outside his den count as a formal challenge. He's bound by his own code to answer it. Then we kill ten of his pack to show the challenge has teeth, and he comes out for the duel."));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*He hands over three iron-shod stakes wrapped in oilcloth and a folded translation of the challenge-rite*{/}"));
						await dialog.Msg(L("Three stakes, hammer each one to half-depth at the den-mouth in a triangle around the entrance. Read the challenge-rite from the paper - even broken Harugal carries the meaning."));
						await dialog.Msg(L("Then ten of his pack. The Elite comes out around the eighth, fully armoured - the challenge requires formal dress. The duel is one-on-one. The surviving Harugals won't interfere."));
						break;

					case "info":
						await dialog.Msg(L("Without the challenge, ambushing him is dishonourable - and the pack avenges dishonourable kills with three generations of vendetta. The militia learned that in the demon war. We lost twelve good hands before somebody read the codex."));
						await dialog.Msg(L("With the challenge, the duel is sealed. Pack scatters whether you win or lose - though it's better for everyone if you win. Harugals respect a lawful kill."));
						break;

					case "leave":
						await dialog.Msg(L("Pack stays bold, the heights stay contested, the militia loses three more hands to Harugal vendetta this season. I'll read the codex to anyone willing. Just has to be someone willing. I'll keep waiting."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*He rolls the codex closed and slides a heavy coin pouch across the rock-table*{/}"));
					await dialog.Msg(L("Stakes planted, challenge sealed, Elite dropped in formal duel. The pack scattered. I watched them go - no signs of a vendetta forming. The codex held."));
					await dialog.Msg(L("Bounty plus a code-stipend. The militia council voted the stipend in after the demon-war losses. You're the first lawful Harugal-kill since they passed it. Drink the first cup for the Elite. He fought like the warrior the codex describes."));
				...
				{
					await dialog.Msg(L("Three stakes first. Half-depth, triangle around the den-mouth, read the rite. The Elite won't fight without the formal challenge sealed."));
				}
				else
				{
					await dialog.Msg(L("Stakes sealed, challenge stands. Now ten of his pack - confirms the challenge has weight. The Elite comes out around the eighth, formally armoured."));
				}
			}
			else if (character.Quests.HasCompleted(questId))
			{
		...
		AddNpc(155146, L("[Militia-Captain] Saulius"), "f_tableland_74", 200, 1000, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A militia-captain at a warrant-ledger spread across a table-rock, weighted with militia-stamped coins*{/}"));
				await dialog.Msg(L("Steel heights are the highest contested ground on the upper terrace. Three species share it - Tiny Brown Mages on the open ridges, Purple Keparis in the brush-thickets, Blue Hohen Gulaks in the cave-mouths."));
				await dialog.Msg(L("My warrant-ledger pays per cleared sweep. The Heights Cairn at the camp edge records each sweep in chalk. The ledger only pays against verified cairn-marks. That's how the militia council keeps contractor-pay honest."));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*He hands over a stub of bright blue chalk and a warrant-token cast in militia bronze*{/}"));
						await dialog.Msg(L("Thirty-six kills, no padding. Patrol-walkers count the corpse-piles on Sunday's perimeter."));
						await dialog.Msg(L("Heights Cairn sits at the camp edge, beside the supply-tent. Chalk a single bracket-mark - one stroke down, one across. Blue chalk because the Heights wind strips white inside a day."));
						break;

					case "info":
						await dialog.Msg(L("The patrol-walkers verify the kill-count Sunday morning. The cairn-mark records the contractor's claim Saturday evening. If the two match, the warrant pays out at noon. If they don't match, the warrant pays the lower number and the contractor gets one warning."));
						await dialog.Msg(L("Two warnings and the contractor's blacklisted from militia work for a year. Three and they're blacklisted from Salvia warrants entirely. Old council rule. We don't play games with warrant-coin."));
						break;

					case "leave":
						await dialog.Msg(L("Plateau stays wild, the warrant goes unpaid, the steel heights eventually empty out. I'll find a closer hand even if I have to recruit from the patrol-walkers - which means thinning my own perimeter for the duration."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*He countersigns the warrant in militia ink and weighs out coin onto the table-rock*{/}"));
					await dialog.Msg(L("Cairn-mark verified, ledger holds. Warrant pays in full - no warnings, no questions. Patrol-walkers reported a clean perimeter at sun-up."));
				...
				{
					await dialog.Msg(L("Sweep done. Now the Heights Cairn - bracket-mark, blue chalk, no flourishes. Patrol-walkers verify it Sunday morning."));
				}
				else
				{
					await dialog.Msg(L("Twelve of each species, no shortcuts. Gulaks in the cave-mouths are the hardest count - they retreat into the dark, you have to go in after them. Bring a torch."));
				}
			}
			else if (character.Quests.HasCompleted(questId))
			{
				await dialog.Msg(L("Militia patrols the heights now, paid by your sweep-warrant. Three new patrol-hands signed on after they read the council's notice about contractors who get the bracket-mark right. Some of them think you're militia-trained, slipped from the rolls. I don't tell them otherwise."));
			}
		});
...
public class FTableland74Quest1002 : QuestScript
{
	protected override void Load()
	{
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver(L("[Alchemist] Tilda"), "f_tableland_74");
...
public class FTableland74Quest1003 : QuestScript
{
	protected override void Load()
	{
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver(L("[Aether-Scholar] Justina"), "f_tableland_74");
...
public class FTableland74Quest1005 : QuestScript
{
	protected override void Load()
	{
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver(L("[Bounty-Sergeant] Tadas"), "f_tableland_74");
