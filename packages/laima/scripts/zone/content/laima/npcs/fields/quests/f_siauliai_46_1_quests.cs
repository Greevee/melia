	...
	protected override void Load()
	{
		...
		AddNpc(20109, L("[Grove-Warden] Rokas"), "f_siauliai_46_1", 770, -50, 90, async dialog =>
		{
			...
			else if (character.Quests.IsActive(questId))
			{
				...
				if (killObj.Done)
				{
					await dialog.Msg(L("{#666666}*He nods, satisfied with the count*{/}"));
		...
		// =====================================================================
		AddNpc(20017, L("[Ink-Maker] Zofija"), "f_siauliai_46_1", -600, -900, 45, async dialog =>
		{
			var character = dialog.Player;
			var questId = new QuestId("f_siauliai_46_1", 1003);

			dialog.SetTitle(L("Zofija"));
		...
		void AddOrbSpot(int spotNumber, int x, int z, int direction)
		{
			AddNpc(47247, L("Infro Still-Pool"), "f_siauliai_46_1", x, z, direction, async dialog =>
			{
				...
				if (result == TimeActionResult.Completed)
				{
					...
					if (currentCount >= 8)
						character.ServerMessage(L("{#FFD700}All orbs gathered! Return to Ink-Maker Zofija.{/}"));
				}
				else
				{
...
public class InfroBloodOrbsQuest : QuestScript
{
	protected override void Load()
	{
		...
		SetType(QuestType.Sub);
		SetDescription("Ink-Maker Zofija needs eight Infro Blood-orbs gathered from still pools for her century-lasting red ink.");
		SetLocation("f_siauliai_46_1");
		SetAutoTracked(true);
		SetReceive(QuestReceiveType.Manual);
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver("[Ink-Maker] Zofija", "f_siauliai_46_1");
