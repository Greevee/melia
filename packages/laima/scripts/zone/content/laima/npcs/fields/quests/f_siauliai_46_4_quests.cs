	...
	protected override void Load()
	{
		...
		AddNpc(20060, L("[Apiary-Keeper] Jogaila"), "f_siauliai_46_4", 1100, -800, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you clear the hives for us?"),
		...
		AddNpc(20114, L("[Chandler] Rasa"), "f_siauliai_46_4", 1200, -850, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you bring me the pods?"),
		...
		AddNpc(20114, L("[Weaver] Audra"), "f_siauliai_46_4", 0, 400, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you bring me the fleece?"),
					Option(L("I'll bring"), "help"),
					Option(L("Feral?"), "info"),
					Option(L("Skip"), "leave")
				);
		...
		//-------------------------------------------------------------------------
		AddNpc(20116, L("[Crystal-Warder] Nora"), "f_siauliai_46_4", 900, 200, 0, async dialog =>
		{
			var character = dialog.Player;
			var questId = new QuestId("f_siauliai_46_4", 1004);

			dialog.SetTitle(L("Nora"));

			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("Rootcrystals bleed dust the bees can't abide. Break twelve for eight dust-sachets."));

				var response = await dialog.Select(L("Will you bring me the sachets?"),
		...
		AddNpc(155146, L("[Militia-Captain] Tauras"), "f_siauliai_46_4", 400, -200, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you sweep the farm?"),
...
public class FSiauliai464Quest1004 : QuestScript
{
	protected override void Load()
	{
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver(L("[Crystal-Warder] Nora"), "f_siauliai_46_4");
