	...
	protected override void Load()
	{
		...
		AddNpc(20109, L("[Chapel-Warden] Vincas"), "f_gele_57_4", 317, 867, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("I need eighteen put back to rest. Don't make a show of it - quick and clean. They were chapel-wards in life. They deserve a quiet end. Can you help?"),
					Option(L("I'll put eighteen to rest"), "help"),
					Option(L("Why are they walking now?"), "info"),
					Option(L("Undead aren't my thing"), "leave")
				);
				...
				switch (response)
				{
					case "help":
						...
						await dialog.Msg(L("Say their name if you know it. If not, just say a chapel word - 'rest', 'return', 'release'. The words matter more than the blade does."));
						break;

					case "info":
						await dialog.Msg(L("A crypt-seal failed. The chapel wardmages take forever to re-bless one - and the Fedimian wardmage stopped answering my letters three weeks ago."));
						await dialog.Msg(L("Until someone redraws the seal, the Rodelin will keep drifting up. We just put them back one by one."));
						break;

					case "leave":
						await dialog.Msg(L("Then take the west path, traveler. The undead are clustered to the east. West keeps you clear."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				if (killObj.Done)
				{
					await dialog.Msg(L("{#666666}*He listens to the count, then bows his head*{/}"));
					await dialog.Msg(L("All eighteen back to rest. The memorial paths will be quiet for a day or two, at least."));
					await dialog.Msg(L("Take this - chapel coin, a warden's share. And don't tell anyone I paid you. The Fedimian accounts don't cover mercy-strokes."));
			...
			{
				await dialog.Msg(L("The Rodelin still drift, but not as much. The seal will hold the rest - if Fedimian ever remembers we exist."));
			}
		});
		...
		AddNpc(20107, L("[Chapel-Steward] Algis"), "f_gele_57_4", 1210, 2016, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				await dialog.Msg(L("Seventeen reliquaries are missing after the crypt-break. Some are probably scattered along the memorial paths. The rest might be in Rodelin hands by now."));
				...
				switch (response)
				{
					...
					case "info":
						await dialog.Msg(L("Our chapel runners are children. The Panto archers on the ridge pick off anyone shorter than their bowstave. I won't send kids out there to be shot at."));
						await dialog.Msg(L("You're a traveler. Travelers take their own risks. I'm not too proud to pay a stranger when my own runners can't get through."));
						break;

					case "leave":
						await dialog.Msg(L("Paper is all the chapel has to send. We can't afford to send people, and we can't wait that long either."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				if (deliverObj.Done)
				{
					...
					await dialog.Msg(L("Here's your pay - a steward's thanks, in coin. And a chapel-charm. It won't fix the crypt, but it'll keep Rodelin hands off your cloak."));
			...
			{
				await dialog.Msg(L("Four reliquaries recovered. I've sent word to the Fedimian magistrates - the rest are their problem now."));
			}
		});
		...
		AddNpc(20117, L("[Ridge-Watch] Emilis"), "f_gele_57_4", -949, 2207, 89, async dialog =>
		{
			...
			else
			{
				...
				await dialog.Msg(L("Travelers come through, I log what they're carrying. You got chapel business?"));
			}
		});
		...
		AddNpc(20017, L("[Garden-Caretaker] Neringa"), "f_gele_57_4", -1102, 679, 90, async dialog =>
		{
			...
			else if (character.Quests.IsActive(questId))
			{
				...
				if (saplingCount >= 8)
				{
					...
					await dialog.Msg(L("Here's a caretaker's coin - small, but honest. And a pot of tea leaves from my own herb-bench. Steep them slow."));
		...
		AddNpc(20151, L("[Memorial-Keeper] Ieva"), "f_gele_57_4", -489, 1995, 180, async dialog =>
		{
			...
			else if (character.Quests.IsActive(questId))
			{
				...
				if (plotsChecked >= 4)
				{
					...
					await dialog.Msg(L("Two outward. Plot Four went first - maybe a season ago, and nobody noticed. Plot Three followed recently. We thought we were hunting one waking. There were two."));
