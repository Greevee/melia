		...
		AddNpc(20017, L("[Forager] Gedas"), "f_huevillage_58_2", -247, -1359, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				switch (response)
				{
					...
					case "info":
						await dialog.Msg(L("My back. Twenty years of bending for husks and my spine's done. I still walk the fields to scout, but I can't bend for pickups anymore."));
						break;
		...
		AddNpc(20151, L("[Naturalist] Audra"), "f_huevillage_58_2", -548, 133, 90, async dialog =>
		{
			...
			else if (character.Quests.IsActive(questId))
			{
				...
				if (sitesChecked >= 4)
				{
					...
					await dialog.Msg(L("Full naturalist's stipend - you've earned it. I'll file the Fedimian census amendment tonight, and this gorge should be on the protected-refuge list by month's end."));
