...
public class FWhitetrees233QuestNpcsScript : GeneralScript
{
	protected override void Load()
	{
		...
		// =====================================================================
		// Novice Marite - First-time adventurer's letter to her mother
		//---------------------------------------------------------------------
		AddNpc(147473, L("[Novice] Marite"), "f_whitetrees_23_3", -537, 344, 0, async dialog =>
		{
			var character = dialog.Player;
			var questId = new QuestId("f_whitetrees_23_3", 1002);

			dialog.SetTitle(L("Marite"));

			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A young adventurer with a letter, surrounded by curious Fragolin*{/}"));
				await dialog.Msg(L("{#666666}*Six nearly identical berry-sprites watch her intently*{/}"));
		...
		AddNpc(147418, L("[Courier] Sigute"), "f_whitetrees_23_3", -552, 1400, 225, async dialog =>
		{
			...
			if (delivered)
			{
				await dialog.Msg(L("The letter's safely in my bag. Tell Marite her mother will have it by tomorrow noon."));
				return;
			}

			await dialog.Msg(L("{#666666}*She accepts the letter and tucks it into an inner pocket*{/}"));
			await dialog.Msg(L("Marite's handwriting - I recognize the loop on the M. Third letter she's sent this month through me."));
			...
			character.ServerMessage(L("{#FFD700}Letter delivered. Return to Marite.{/}"));
		});
...
public class FirstLetterHomeQuest : QuestScript
{
	protected override void Load()
	{
		...
		SetType(QuestType.Sub);
		SetDescription("Novice Marite needs her letter home carried to Courier Sigute at the Orsha warp - she can't bring herself to walk back yet.");
		SetLocation("f_whitetrees_23_3");
		SetAutoTracked(true);
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver("[Novice] Marite", "f_whitetrees_23_3");

		AddObjective("deliverLetter", "Deliver Marite's letter to Courier Sigute",
