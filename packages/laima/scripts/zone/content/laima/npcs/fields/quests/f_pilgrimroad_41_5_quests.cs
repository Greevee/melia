...
public class FPilgrimroad415QuestNpcsScript : GeneralScript
{
	protected override void Load()
	{
		...
		AddNpc(20060, L("[Pilgrim-Warden] Mindaugas"), "f_pilgrimroad_41_5", -250, -50, 0, async dialog =>
		{
			...
			{
				await dialog.Msg(L("Brown Nukas are swarming Ouaas's road. Kill forty of them and the pilgrims can pass through safely."));

				var response = await dialog.Select(L("Will you make the road safe again?"),
				...
				switch (response)
				{
					case "help":
						...
						await dialog.Msg(L("Forty of them. Watch out for their charge."));
						break;

					case "info":
						await dialog.Msg(L("The Salvia faithful walk this road to reach the memorial. The Nukas trample them."));
						break;

					case "leave":
						await dialog.Msg(L("Road stays unsafe, then."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				if (killObj.Done)
				{
					await dialog.Msg(L("Pilgrims will be able to cross the road tonight."));
			...
			{
				await dialog.Msg(L("The first caravan reached the memorial."));
			}
		});
		...
		AddNpc(20117, L("[Fletcher] Algirdas"), "f_pilgrimroad_41_5", 1100, 200, 270, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("The Brown Lapasape archers carry fire-hardened arrowheads. Kill twenty-eight and bring me seven of them."));

				var response = await dialog.Select(L("Will you bring me the arrowheads?"),
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("Just the heads. The shafts splinter on impact."));
						break;

					case "info":
						await dialog.Msg(L("I refit them for the memorial guard. Cheaper than forging new ones."));
						break;

					case "leave":
						await dialog.Msg(L("Then the guard runs short."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("Seven heads. The quivers will be refilled."));
			...
			{
				await dialog.Msg(L("The memorial guard's fully equipped."));
			}
		});

		// Quest 3: Yellow-Eyed Petals
		//-------------------------------------------------------------------------
		AddNpc(20114, L("[Memorial-Tender] Olita"), "f_pilgrimroad_41_5", -300, 870, 90, async dialog =>
		{
			var character = dialog.Player;
			var questId = new QuestId("f_pilgrimroad_41_5", 1003);

			dialog.SetTitle(L("Olita"));

			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("Red Elma bloom with yellow-eyed petals along the verge. Kill eighteen and gather me six petals."));

				var response = await dialog.Select(L("Will you gather the petals for me?"),
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("Press them flat. Don't crush them."));
						break;

					case "info":
						await dialog.Msg(L("They're for the memorial offering. Ouaas favoured yellow."));
						break;

					case "leave":
						await dialog.Msg(L("The altar stays bare, then."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("Six petals. I'll dress the altar at dusk."));
			...
			{
				await dialog.Msg(L("The pilgrims wept at the offering."));
			}
		});
		...
		AddNpc(20118, L("[Road-Marshal] Kestutis"), "f_pilgrimroad_41_5", 700, 380, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A road-marshal beside a pile of recovered black-fletched arrows*{/}"));
				await dialog.Msg(L("Three pilgrims this week. Two on foot, one barefoot widow walking the memorial circuit. All three shot from above before they ever saw the archer."));
				await dialog.Msg(L("Deadbornscab nest in the cliff-roosts above the bend. They wait in the shadows, then volley-shoot when a pilgrim walks through the open stretch. We've never had an archer problem this bad on Ouaas's road."));

				var response = await dialog.Select(L("Kill 18 Deadbornscab archers, then spike the four cliff-roosts shut with the iron tacks at the cliff-base. They can't re-nest in spiked wood. Will you do it?"),
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*He gives you a heavy pouch of iron tacks*{/}"));
						await dialog.Msg(L("Eighteen archers, no shortcuts. Kill them in the open if you can - on the cliff, the wind throws every shot off."));
						await dialog.Msg(L("Then the four roosts. Drive a tack through the perch-wood at four points, then a fifth at the entrance. Spiked wood is like poison to nesting Deadbornscab. They won't re-roost for a year."));
						break;

					case "info":
						await dialog.Msg(L("It's memorial-week. Salvia's pilgrims come thicker than usual to lay flowers for Ouaas, and the Deadbornscab know it. They feed on the panic as much as the kill."));
						await dialog.Msg(L("If we clear the bend before the second-week processions, the rest of the memorial passes safe. If we don't, the widow count doubles."));
						break;

					case "leave":
						await dialog.Msg(L("Then the cliff-bend stays a death-trap and the widow count grows. I'll be here, pulling arrows out of straw and pretending it's enough."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*He almost smiles when he weighs the empty tack-pouch*{/}"));
					await dialog.Msg(L("The cliffs are silent. All four roosts spiked clean through. The bend will hold for at least a season, maybe two."));
				...
				{
					await dialog.Msg(L("Eighteen archers first. Roosts can wait until they stop shooting - no point spiking wood while a Deadbornscab is still drawing on you from above."));
				}
				else
				{
					await dialog.Msg(L("Cliffs are quiet. Now spike the four roosts - five tacks each, perch and entry. The poison in the iron does the rest."));
				}
			}
			else if (character.Quests.HasCompleted(questId))
			{
		...
		AddNpc(47245, L("[Hunter] Vytenis"), "f_pilgrimroad_41_5", -850, -400, 90, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A hunter sketching hoof-shapes onto a slate*{/}"));
				await dialog.Msg(L("There's a Bull Nuka out there. Twice the size of his herd, scarred along the flank from a wagon-pole he charged through last spring. The pilgrims call him the Trampler - the carriages he breaks aren't always empty when he hits them."));
				await dialog.Msg(L("I've chalked three of his tracks along the road. Read each one - the splay tells me which way his herd's drifting next. After that we kill enough of his cows to bait him out."));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*He gives you the slate and a charcoal stub*{/}"));
						await dialog.Msg(L("Three tracks - north verge, south crook, east scree. At each one, sketch the splay-angle on the slate. The deeper hoof tells you which way he's leaning when he runs."));
					...
					case "info":
						...
						await dialog.Msg(L("The other half is making sure the cows don't pick a worse bull from his bloodline. The tracks tell us which yearling has been following close. That's the next bull - we watch him, but we don't hunt him."));
						break;

					case "leave":
						await dialog.Msg(L("Then the Trampler keeps trampling and the slate stays half-sketched. I'll be here, drawing tracks I can't get to."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*He copies your sketches into his leather log-book*{/}"));
					await dialog.Msg(L("Bull's down, herd-drift logged. Salvia's hunter-guild has the next ninety years of road-data, thanks to you."));
				...
				{
					await dialog.Msg(L("Three chalked tracks first. Sketch the splay-angle on each one onto the slate. The bull moves through that drift-line, and if we miss it, we miss the next one."));
				}
				else
				{
					await dialog.Msg(L("Tracks are logged. Now kill ten Nukas - he won't take the bait for less, and the cows reform a herd if you stop short."));
				}
			}
			else if (character.Quests.HasCompleted(questId))
			{
				await dialog.Msg(L("The herd broke into three small drifts, each led by a younger cow. The yearling I had my eye on - the next bull - he stayed with the south drift. We watch him from here on, but we don't hunt him. That's the deal we have with the herd."));
			}
		});
		...
		AddNpc(155146, L("[Militia-Captain] Gediminas"), "f_pilgrimroad_41_5", 450, -300, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A militia-captain with a memorial-week calendar*{/}"));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*She gives you a wax-sealed wick from her belt*{/}"));
			...
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*She counts coin into a memorial-stamped pouch*{/}"));
...
public class FPilgrimroad415Quest1003 : QuestScript
{
	protected override void Load()
	{
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver(L("[Memorial-Tender] Olita"), "f_pilgrimroad_41_5");
