	...
	protected override void Load()
	{
		...
		AddNpc(20060, L("[Mesa-Warden] Aistis"), "f_tableland_71", 200, 300, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you open the mesa for the surveyors?"),
		...
		AddNpc(20114, L("[Fletcher] Rasa"), "f_tableland_71", 600, 400, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you bring me the fletches?"),
		...
		//-------------------------------------------------------------------------
		AddNpc(20116, L("[Herbalist] Saule"), "f_tableland_71", -400, 600, 0, async dialog =>
		{
			var character = dialog.Player;
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you bring me the resin?"),
					Option(L("I'll bring"), "help"),
		...
		void AddRidgeCairn(int cairnNumber, int x, int z, int direction)
		{
			AddNpc(47190, L("Ridge-Cairn"), "f_tableland_71", x, z, direction, async dialog =>
			{
				...
				{
					await dialog.Msg(L("{#666666}*Already stacked. The cairn is finished*{/}"));
					return;
				}
		...
		AddNpc(47190, L("Mesa Survey Stone"), "f_tableland_71", 1350, 850, 90, async dialog =>
		{
			...
			{
				await dialog.Msg(L("{#666666}*Your flag is set in the stone-socket*{/}"));
				return;
			}
...
public class FTableland71Quest1003 : QuestScript
{
	protected override void Load()
	{
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver(L("[Herbalist] Saule"), "f_tableland_71");
