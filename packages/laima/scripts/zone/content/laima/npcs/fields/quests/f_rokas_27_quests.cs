...
public class FRokas27QuestNpcsScript : GeneralScript
{
	protected override void Load()
	{
		...
		AddNpc(20017, L("[Relic-Sweeper] Aldona"), "f_rokas_27", 950, -700, 180, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A grey-haired sweeper sifting dust through a horsehair sieve*{/}"));
				...
				{
					case "help":
						await dialog.Msg(L("{#666666}*She gives you a second sieve from her gather-pack*{/}"));
			...
			else if (character.Quests.IsActive(questId))
			{
				...
				if (fragmentCount >= 8)
				{
					await dialog.Msg(L("{#666666}*She checks each fragment against a worn reliquary-list*{/}"));
					...
					await dialog.Msg(L("{#666666}*She gives you a leather pouch and a polished ridge-stone*{/}"));
		...
		AddNpc(20151, L("[Tomb-Scholar] Vytenis"), "f_rokas_27", 1440, 400, 90, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A young scholar in road-worn academic grey, with a ridge-survey weighted down by tomb-dust-stained stones*{/}"));
				...
				{
					case "help":
						await dialog.Msg(L("{#666666}*He marks four points on a torn survey-page in ridge-chalk*{/}"));
			...
			else if (character.Quests.IsActive(questId))
			{
				...
				if (tombsChecked >= 4)
				{
					await dialog.Msg(L("{#666666}*He writes rapidly as he listens, then sets the pen down*{/}"));
