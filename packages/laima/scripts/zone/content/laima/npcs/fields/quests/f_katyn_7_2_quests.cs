...
public class FKatyn72QuestNpcsScript : GeneralScript
{
	protected override void Load()
	{
		...
		// Quest 1006: Vakaris — Vytautas's second, soul torn from his petrified body in Dingofasil
		//-------------------------------------------------------------------------
		// TODO: implement secret path quest. Vakaris guides the player to a hidden
		// passage in the forest leading to a "Secret Room" map where the heretics
		// who tore his soul from his body are hiding. Quest objectives, the path
		// reveal mechanic, and the Secret Room map itself are not yet built.
		AddGhostNpc(155132, L("[Restless Soul] Vakaris"), "f_katyn_7_2", 2347, -2545, 0, async dialog =>
		{
			var character = dialog.Player;
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("...Vytautas... Vytautas... do you hear me yet... Vytautas..."));
				await dialog.Msg(L("{#888888}*The ghost stares past you, lips barely moving. He keeps whispering one name, over and over.*{/}"));

				var response = await dialog.Select(L("..."),
					Option(L("Who is Vytautas?"), "info_vytautas"),
					Option(L("Why are you here?"), "info_self"),
					Option(L("What do you want?"), "info_want"),
					Option(L("Leave"), "leave")
				);

				if (response == "info_vytautas")
				{
					await dialog.Msg(L("My captain... My inquisitor... Twenty-three years I bled for him... Twenty-three years he held my oath... He thinks I stand petrified at his cordon... He salutes my body and does not know I am here..."));
					await dialog.Msg(L("They tore me out of myself, the choir did. Stone for the body. Soul for the offering. They thought no one would notice."));
				}
				else if (response == "info_self")
				{
					await dialog.Msg(L("Vakaris of the Sanctified Tribunal... Second of Inquisitor Vytautas's blade... Reading the writ at the Dingofasil cordon when the rite caught me mid-word..."));
					await dialog.Msg(L("My body stands there yet. Stone, sword still drawn. But my soul... My soul they pulled here, to this forest, to feed something behind a door they hid in the trees."));
				}
				else if (response == "info_want")
				{
					await dialog.Msg(L("Vengeance. On the choir. On the door. On whatever they bound me to feed."));
					await dialog.Msg(L("There is a path through these trees. Hidden. The heretics walk it when they come for souls like mine. I have watched them pass and I know the markings."));
					await dialog.Msg(L("Help me open it. Help me walk it. Help me find what waits at the end and put my blade through its throat — and then tell Vytautas where I truly fell, so he can salute me properly."));

					var r2 = await dialog.Select(L("..."),
						Option(L("I'll help you open the path."), "help"),
						Option(L("Not yet."), "leave")
					);

					if (r2 == "help")
					{
						// TODO: replace placeholder with real Quest 1006 acceptance once the
						// secret path objectives + Secret Room map are implemented.
						await dialog.Msg(L("{#888888}*Vakaris's whispering falters for a moment, as if he heard you. Then it resumes — Vytautas, Vytautas, Vytautas — fainter now, threaded with something like hope.*{/}"));
						await dialog.Msg(L("...soon, my captain... soon..."));
					}
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				// TODO: handle in-progress objectives for the secret-path quest.
				await dialog.Msg(L("...the path... walk the path... Vytautas... Vytautas..."));
			}
			else if (character.Quests.HasCompleted(questId))
			{
				// TODO: post-quest dialog after the Secret Room is cleared and Vakaris is freed.
				await dialog.Msg(L("Tell him... Tell Vytautas where I truly fell... Tell him I bled for him to the last..."));
			}
		});
	}
}
...
// TODO: Quest 1006 — Vakaris's secret path. Replace this stub with real
// objectives once the Secret Room map exists and the path-reveal mechanic is
// designed (e.g. find heretic markings in the forest, decipher them, open the
// hidden door, clear the Secret Room, return to Vakaris).
public class FKatyn72Quest1006 : QuestScript
{
	protected override void Load()
	{
		SetId("f_katyn_7_2", 1006);
		SetName(L("Vakaris's Vengeance"));
		SetType(QuestType.Sub);
		SetDescription(L("A torn soul of the Sanctified Tribunal whispers his captain's name and begs for a path through the forest to the heretics who unmade him."));
		SetLocation("f_katyn_7_2");
		SetAutoTracked(true);
		SetReceive(QuestReceiveType.Manual);
		SetCancelable(true);
		...
		// TODO: real objectives — find heretic markings, open the hidden path,
		// clear the Secret Room, return to Vakaris.
		AddObjective("placeholder", L("Open the hidden path through the forest"),
			new KillObjective(1, new[] { MonsterId.Sakmoli }));

		// TODO: rewards balanced for the Secret Room boss tier; current values
		// are inherited placeholders from the previous quest.
		AddReward(new ExpReward(23800, 16200));
		AddReward(new SilverReward(17000));
		AddReward(new ItemReward(640086, 2));
		AddReward(new ItemReward(640004, 3));
