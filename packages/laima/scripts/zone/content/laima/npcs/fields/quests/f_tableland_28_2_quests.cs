	...
	protected override void Load()
	{
		...
		AddNpc(20060, L("[Sky-Ward] Algirdas"), "f_tableland_28_2", 0, 0, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you clear the sky for us?"),
		...
		AddNpc(20114, L("[Fletcher] Grazina"), "f_tableland_28_2", -800, 500, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you bring me the feathers?"),
		...
		AddNpc(20117, L("[Bowyer] Ramunas"), "f_tableland_28_2", 400, 800, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				var response = await dialog.Select(L("Will you bring me the sinews?"),
					Option(L("I'll bring them"), "help"),
		...
		AddNpc(20116, L("[Shepherd] Ausra"), "f_tableland_28_2", -500, -1400, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A shepherd in a long oilcloth coat with a small flock filing past*{/}"));
				await dialog.Msg(L("My family's run this pasture for three generations. Stogas grass is the best on the tableland - sheep grow fat on it, wool grows long, and the wool-merchants pay above market every spring."));
				await dialog.Msg(L("Then the Blue Lapasapes came down out of the salt-flats. They strip grass in long rows. One minute the pasture's green, the next it's bare earth and the sheep are wandering around confused."));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*She hands over a length of leather strap from her belt*{/}"));
						...
						await dialog.Msg(L("Bell-posts are wooden stakes in the four pasture-corners. Stand them upright, lash the bell to the head with the strap I gave you. The bell needs to swing loose - if it's tight, the flock can't hear it."));
						break;

					case "info":
						await dialog.Msg(L("Move the pasture? My grandfather laid a foundation here. The shepherd's hut, the lambing-shed, the wool-press - rebuilding all that would cost more than three years of mutton-money. The land's part of the family."));
						await dialog.Msg(L("And Stogas grass doesn't grow anywhere else. There's no second pasture. Just this one, and we defend it."));
						break;

					case "leave":
						await dialog.Msg(L("Then the flock starves slow and the family pasture goes back to whatever the Lapasapes leave behind. I'll count my sheep tonight like I always do. There might be fewer tomorrow."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*The dusk-wind catches each of the four bells in turn*{/}"));
					await dialog.Msg(L("Hear that? Four bells, four corners, the flock answering each one. Pasture's resting, the strays are finding their way home before dark. Sheep'll be fat again by month-end."));
					await dialog.Msg(L("Take this. Shepherd's purse, plus a wool-skein from last spring's clipping - the softest one I had. Tell the wool-merchant I sent it. He'll pay double for it on principle."));
					character.Quests.Complete(questId);
				}
				else if (!killObj.Done)
				{
					await dialog.Msg(L("Thirty-five Lapasapes first. Bell-posts won't help if the flock keeps getting trampled at the corners."));
				}
				else
				{
					await dialog.Msg(L("Lapasapes are pushed back. Now the four bell-posts - upright, leather strap, bell loose enough to ring. Dusk-wind's coming."));
				}
			}
			else if (character.Quests.HasCompleted(questId))
			{
				await dialog.Msg(L("Flock's fat, wool prices up, four bells ringing at dusk. The wool-merchant came by last spring and asked who I owed. I told him. He said you bought him three years of premium clip with one afternoon's work. He'd like to meet you. So would I, again."));
			}
		});
		...
		//-------------------------------------------------------------------------
		AddNpc(47245, L("[Falconer] Algis"), "f_tableland_28_2", 1200, 1200, 0, async dialog =>
		{
			var character = dialog.Player;
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A falconer with a small kestrel hooded on her glove*{/}"));
				await dialog.Msg(L("The Siaulav King rules the upper winds above the tableland. Twenty-foot wingspan, claws the size of kitchen knives, soars for hours on the cliff-edge thermals without a wingbeat."));
				await dialog.Msg(L("My falcons can't share the sky with him. I've cut three thermal-banners that sit at the cliff edges. Drive them in and the canvas breaks the updraft he rides. Then we thin his flock and he dives to challenge whatever grounded him."));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*She turns her attention from the kestrel to you*{/}"));
						await dialog.Msg(L("Three banners, three cliff edges. Drive the pole deep, lash the canvas open. Wind catches it and the thermal collapses inside an hour."));
						await dialog.Msg(L("Then ten birds. He dives around the eighth - it'll feel like a portcullis dropping. Step sideways at the last second. His momentum carries him past you. Strike his throat as he banks."));
						break;

					case "info":
						await dialog.Msg(L("Twenty-foot wingspan, claws the size of kitchen knives, beak that shears through bronze. He's killed three of my falcons in the past six months. All caught in his thermal-shadow before they could climb."));
						await dialog.Msg(L("He doesn't hunt because he's hungry. He hunts for territory. The upper winds are his and he reminds everyone of it every dawn. We can't share the sky with that. Either he goes or my falcons stay grounded forever."));
						break;

					case "leave":
						await dialog.Msg(L("His flock's splitting up already - some have moved south. He hasn't followed. He won't leave the cliff-thermals. The kestrel and I will keep ground-hunting until you change your mind. We'll eat thinner for it."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*She unhoods the kestrel and lifts the glove; the bird climbs into the empty sky*{/}"));
					await dialog.Msg(L("Banners standing, King down, kestrel climbing. Watch her. First time in six months she's been able to take that thermal without watching for a shadow above her."));
					await dialog.Msg(L("Falconer's purse, plus a King-claw. Mounted, it makes a fine cloak-clasp. Wear it where another falconer can see it. Tells the trade you're one of us now."));
				...
				{
					await dialog.Msg(L("Three banners first. Cliff edges, pole deep, canvas open. Thermal collapses inside the hour - he can't dive without being grounded first."));
				}
				else
				{
					await dialog.Msg(L("Banners up, his thermals gone. Now ten birds. He dives at the eighth - portcullis drop, sidestep, hit his throat as he banks."));
				}
			}
			else if (character.Quests.HasCompleted(questId))
			{
				await dialog.Msg(L("Sky's quiet. My falcons hunt the upper winds in pairs now. They couldn't before - the King would split them. Three new chicks fledged this season. One of them I named for you, since you wouldn't give me your real name."));
			}
		});
		...
		AddNpc(155146, L("[Caravan Master] Leokadija"), "f_tableland_28_2", -1200, 1000, 0, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				await dialog.Msg(L("{#666666}*A caravan-master folds a route-map and ties it shut with waxed twine*{/}"));
				await dialog.Msg(L("Stogas plateau is the southern leg of every wool-caravan running between the tableland and Klaipeda. The route's profitable, but only if the staging-post stays viable. And the staging-post only stays viable if the surrounding pasture stays cleared."));
				await dialog.Msg(L("Standard sweep clears the route. The Caravan-Stone at the staging-post records each sweep in chalk. The next caravan-master reads the stone before the wagons roll. Without a chalk-line, they pad guard-pay by a third."));
				...
				switch (response)
				{
					case "help":
						character.Quests.Start(questId);
						await dialog.Msg(L("{#666666}*She hands over a stub of red caravan-chalk, brighter than the standard white*{/}"));
						await dialog.Msg(L("Twenty-four kills, no padding. Next caravan-master cycles through every week and counts the corpse-piles against my ledger."));
						await dialog.Msg(L("Caravan-Stone sits at the staging-post bend. One horizontal line, red chalk - that's the Stogas convention. The next caravan-master reads it as 'sweep done by a contractor I trust'. Lying about a sweep gets you blacklisted."));
						break;

					case "info":
						await dialog.Msg(L("A ledger travels with the master. The Caravan-Stone stays where the next master needs it. Half my staging-posts have stones older than my mother. The chalk layers build up like rings on a tree-stump."));
						await dialog.Msg(L("Nobody designed it. It just worked. Caravan-traders are practical people. We keep what works."));
						break;

					case "leave":
						await dialog.Msg(L("Caravan still rolls. We pay double-guard, wool gets more expensive in Klaipeda, the merchants raise prices, the route loses customers. All for one missing sweep. I'll walk till I find a closer hand if I have to."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				{
					await dialog.Msg(L("{#666666}*She countersigns a warrant in matching red ink and seals it*{/}"));
					...
					await dialog.Msg(L("Stop by next fortnight if you want the contract again. The route's always hiring. The caravan-trade thanks a swordhand who chalks the stone like they mean it."));
					character.Quests.Complete(questId);
				}
				else if (sObj.Done && lObj.Done)
				{
					await dialog.Msg(L("Sweep complete. Now the Caravan-Stone - red chalk, single horizontal line, no flourishes. Next master reads it at first light."));
				}
				else
				{
			...
			{
				await dialog.Msg(L("Caravan through, single-guard rate, wool-merchants happy. Your red chalk-line on the Caravan-Stone is the third-most-recent on the eastern face. Some traders ask who chalked it. I tell them a swordhand who knows the value of a stone-line."));
			}
		});
...
public class FTableland282Quest1005 : QuestScript
{
	protected override void Load()
	{
		...
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver(L("[Falconer] Algis"), "f_tableland_28_2");
