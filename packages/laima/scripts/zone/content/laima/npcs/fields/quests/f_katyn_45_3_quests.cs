...
public class FKatyn453QuestNpcsScript : GeneralScript
{
	protected override void Load()
	{
		...
		// Quest 1003: Dream-Diviner Galina — ALIVE, powerful adventurer fighting spirits
		//---------------------------------------------------------------------
		AddNpc(20151, L("[Spirit-Hunter] Galina"), "f_katyn_45_3", 953, -786, 0, async dialog =>
		{
			var character = dialog.Player;
			var questId = new QuestId("f_katyn_45_3", 1003);
			dialog.SetTitle(L("Galina"));
...
public class JellyfishTearFrondsQuest : QuestScript
{
	protected override void Load()
	{
		...
		SetType(QuestType.Sub);
		SetDescription("Spirit-Hunter Galina needs eight tear-fronds cut from drifting forest-jellyfish for her dream-divinations.");
		SetLocation("f_katyn_45_3");
		SetAutoTracked(true);
		SetReceive(QuestReceiveType.Manual);
		SetCancelable(true);
		SetUnlock(QuestUnlockType.AllAtOnce);
		AddQuestGiver("[Spirit-Hunter] Galina", "f_katyn_45_3");
