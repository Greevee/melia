...
public class FHuevillage583QuestNpcsScript : GeneralScript
{
	protected override void Load()
	{
		...
		//-------------------------------------------------------------------------
		AddNpc(20114, L("[Fletcher] Daiva"), "f_huevillage_58_3", -1064, -705, 45, async dialog =>
		{
			var character = dialog.Player;
		...
		AddNpc(147473, L("[Huntress] Giedre"), "f_huevillage_58_3", 1735, -544, 90, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				switch (response)
				{
					...
					case "info":
						await dialog.Msg(L("Doyors avoid bone. Old hunter trick - pack a cairn with deer-bone and they'll route around it. They don't care about plain stone. Bone tells them a hunter's been on the trail."));
						await dialog.Msg(L("The chalk's for the patrols, not the Doyors. One cairn, two messages. Saves walking the trail twice."));
						break;

					case "leave":
						await dialog.Msg(L("It is the village's fight - and the village asked me, and now I'm asking you. The next kid who goes missing won't have a name yet either."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			{
				...
				if (killObj.Done && tObj.Done)
				{
					...
					await dialog.Msg(L("Take this. Huntress's purse, plus a stone the children's mother sent along for whoever did this. She didn't say anything else. She didn't have to."));
					character.Quests.Complete(questId);
				}
		...
		AddNpc(147418, L("[Bounty Hunter] Una"), "f_huevillage_58_3", 152, -761, 90, async dialog =>
		{
			...
			if (!character.Quests.Has(questId))
			{
				...
				await dialog.Msg(L("He's old enough to remember when humans were hunted, not the hunters. The protector instinct's strong in him - kill enough of his territory's smaller beasts and he'll come down to deal with it personally."));
				...
				switch (response)
				{
					...
					case "info":
						await dialog.Msg(L("Upent territory-marking is older than any cult. The musk-stone basically says 'this is my ground and I'll protect what lives here.' Scattering them tells him a rival's moved in."));
						await dialog.Msg(L("He has to answer a rival in person. A hunter he could ignore. But a claim on his territory? No way."));
						break;

					case "leave":
						await dialog.Msg(L("It is old-hunter work. Most of the old hunters are dead now. Cobalt's quieter without them. The Upent isn't."));
						break;
				}
			}
			else if (character.Quests.IsActive(questId))
			...
			{
				await dialog.Msg(L("Cobalt's quiet again. The Upent's musk-line is gone, and new Caros are nesting where his territory used to be. The forest moves on quick."));
			}
		});
