...
namespace Melia.Zone.Skills.Handlers.Clerics.Zealot
{
	...
	/// "Temper the Flame".
	/// The way back up the ladder: each press climbs one stage, heals to it
	/// and lashes out with the flame it just compressed. Climbing is an
	/// offensive move, not a retreat — the health it puts back above the
	/// stage is exactly what the fire eats next, and burned health is what
	/// makes every Pyre lash land harder. Fanaticism can then drive you
	/// straight back down.
	/// Every press leaves an ember that keeps the damage bonus of the stage
	/// just left for ten seconds (see Ember_Buff).
	...
	public class Zealot_InvulnerableOverride : ISelfSkillHandler
	{
		...
		/// <summary>
		/// How wide the flame lashes when it is compressed back up a stage.
		/// PLACEHOLDER.
		/// </summary>
		...
		public void Handle(Skill skill, ICombatEntity caster, Position originPos, Direction dir)
		{
			...
			// Captured before the floor moves: the ember keeps the bonus of
			// the stage the Zealot was standing in.
			var stageBonus = ZealotBurnFloor.GetStageBonusPercent(ZealotBurnFloor.GetStage(caster));

			// One rung up, every time. At the top there is nowhere left to
			// climb, but the heal still tops the Zealot off — which is a full
			// load of health for the fire to eat, so the press is never dead.
			...
			_ = caster.PlayEffectToGround("F_explosion051_fire", caster.Position, 1.3f, duration: 900f);

			Send.ZC_NORMAL.PlayTextEffect(caster, caster, "SHOW_CUSTOM_TEXT", 0,
				$"Tempered to {newFloor}%"
				+ (healed > 0 ? $"  +{healed} HP" : "")
